...
const Invest = () => {
  ...
  if (loading || authLoading) return <div className="p-8 text-center">Loading Investment Portfolio...</div>;
  if (error) return <div className="p-8 text-center text-red-500">{error}</div>;
  if (!portfolio) return <div className="p-8 text-center">Could not find your portfolio.</div>

  const totalCost = portfolio.holdings?.reduce((acc, h) => acc + (h.quantity * h.purchasePrice), 0) || 0;
  const totalGain = portfolio.totalValue - totalCost;
              ...
              <div>
                ...
                <div className="bg-white shadow overflow-hidden sm:rounded-md">
                  <ul role="list" className="divide-y divide-gray-200">
                    ...
                    )) : <p className="px-4 py-4 sm:px-6 text-gray-500">You have no holdings yet. Click 'Buy' in the markets list to get started.</p>}
                  </ul>
                </div>
              </div>
            </div>
